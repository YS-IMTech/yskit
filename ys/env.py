...
def retrieve_globals(verbose=False):
    # locate and set G to the globals which directly `import ys`. (only once)
    ...
    while frame_id < len(stack):
        ...
        if "ys" in g:
            G = g
            if verbose:
                print(f"[ys-INFO] located global frame at {frame_id}")
            break
        frame_id += 1
    if G is None:
        raise RuntimeError(
            "Cannot locate global frame, make sure you called exactly `import ys`!"
        )
...
def try_import(target, sources, verbose=False):
    ...
    if target in G:
        if verbose:
            print(f"[ys-INFO] {target} is already present, skipped.")
        return
    ...
    for source in sources:
        try:
            if verbose:
                print(f"[ys-INFO] try to import {source}")
            ...
            if verbose:
                print(f"[ys-INFO] succeed to import {source} as {target}")
            break

        except ImportError as e:
            print(f"[ys-WARN] failed to import {source} as {target}: {str(e)}")
...
usage:
    ys.env() # setup base env
    ys.env('torch') # pytorch with all regular dependencies
    ys.env('[torch]') # pytorch only
"""
...
def env(*packs, verbose=False):
    ...
    def check_pack(pack):
        if pack not in LIBS:
            raise RuntimeError(
                f"[Ys-ERROR] unknown pack {pack}, availables: {list(LIBS.keys())}"
            )
